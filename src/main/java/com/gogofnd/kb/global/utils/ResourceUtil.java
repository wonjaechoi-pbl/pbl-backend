...
public class ResourceUtil {
    ...
    public static String createFilePath(String type,String id,MultipartFile file){
        return "/images/" + type + "/" + id + "/" + file.getOriginalFilename();
    }


    public static String busNoImageSave(MultipartFile file) throws IOException {
        String extension = FilenameUtils.getExtension(file.getOriginalFilename());

        String loc = "/var/www/html/goPay/img/bizNum/"+file.getOriginalFilename();

        File targetFile = new File(Objects.equals(extension, LIVE_PHOTO_EXE) ? loc.replace(LIVE_PHOTO_EXE, "jpg") : loc);

        File folder = new File("/var/www/html/goPay/img/bizNum");
